...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of bchaind is "bchaind". If no
...
func (c *Config) SetDefaultConfig() (error){
	...
	c.HTTPModules = append(c.HTTPModules,"bchain","personal","txpool", "blockproducer")
...
func (c *Config) NodeName() string {
	name := c.NameValue()
	// Backwards compatibility: previous versions used title-cased "Bchaind", keep that.
	if name == "bchaind" || name == "bchaind-testnet" {
		name = "Bchaind"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "bchaind" instances.
var isOldBchaindResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by bchaind 1.4 are used if they exist.
	if c.NameValue() == "bchaind" && isOldBchaindResource[path] {
		oldpath := ""
		if c.Name == "bchaind" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-bchain-keystore")
		ephemeral = keydir
	}
