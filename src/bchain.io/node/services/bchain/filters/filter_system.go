...
//
// @File: filter_system.go
// @Date: 2018/05/08 18:02:08
////////////////////////////////////////////////////////////////////////////////

// Package filters implements an bchain filtering system for block,
...
func (es *EventSystem) SubscribeLogs(crit FilterCriteria, logs chan []*transaction.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeProducedPendingLogs(crit FilterCriteria, logs chan []*transaction.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit FilterCriteria, logs chan []*transaction.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit FilterCriteria, logs chan []*transaction.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
// transactions that enter the transaction pool.
func (es *EventSystem) SubscribePendingTxEvents(hashes chan types.Hash) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) lightFilterNewHead(newHeader *block.Header, callBack func(*block.Header, bool)) {
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.IntVal.Uint64() >= newh.Number.IntVal.Uint64() {
			oldHeaders = append(oldHeaders, oldh)
			oldh = blockchain.GetHeader(es.backend.ChainDb(), oldh.ParentHash, oldh.Number.IntVal.Uint64()-1)
		}
		if oldh.Number.IntVal.Uint64() < newh.Number.IntVal.Uint64() {
			newHeaders = append(newHeaders, newh)
			newh = blockchain.GetHeader(es.backend.ChainDb(), newh.ParentHash, newh.Number.IntVal.Uint64()-1)
			if newh == nil {
...
func (es *EventSystem) lightFilterLogs(header *block.Header, addresses []types.Address, topics [][]types.Hash, remove bool) []*transaction.Log {
	if bloomFilter(header.Bloom, addresses, topics) {
		...
		var unfiltered []*transaction.Log
...
func (es *EventSystem) eventLoop() {
	var (
		...
		// Subscribe []*types.Log
		logsCh  = make(chan []*transaction.Log, logsChanSize)
