...
type downloadTester struct {
	...
	genesis *block.Block       // Genesis blocks used by the tester and peers
	stateDb database.IDatabase // Database used by the tester for syncing from peers
	peerDb  database.IDatabase // Database of the peers containing all data

	ownHashes   []types.Hash                        // Hash chain belonging to the tester
	ownHeaders  map[types.Hash]*block.Header        // Headers belonging to the tester
	ownBlocks   map[types.Hash]*block.Block         // Blocks belonging to the tester
	ownReceipts map[types.Hash]transaction.Receipts // Receipts belonging to the tester
	ownChainTd  map[types.Hash]*big.Int             // Total difficulties of the blocks in the local chain

	peerHashes   map[string][]types.Hash                        // Hash chain belonging to different test peers
	peerHeaders  map[string]map[types.Hash]*block.Header        // Headers belonging to different test peers
	peerBlocks   map[string]map[types.Hash]*block.Block         // Blocks belonging to different test peers
	peerReceipts map[string]map[types.Hash]transaction.Receipts // Receipts belonging to different test peers
...
func newTester() *downloadTester {
	testdb, _ := database.OpenMemDB()
	genesis := genesis.GenesisBlockForTesting(testdb, testAddress, big.NewInt(1000000000))

	tester := &downloadTester{
		genesis:           genesis,
		peerDb:            testdb,
		ownHashes:         []types.Hash{genesis.Hash()},
		ownHeaders:        map[types.Hash]*block.Header{genesis.Hash(): genesis.Header()},
		ownBlocks:         map[types.Hash]*block.Block{genesis.Hash(): genesis},
		ownReceipts:       map[types.Hash]transaction.Receipts{genesis.Hash(): nil},
		ownChainTd:        map[types.Hash]*big.Int{genesis.Hash(): genesis.Number()},
		peerHashes:        make(map[string][]types.Hash),
		peerHeaders:       make(map[string]map[types.Hash]*block.Header),
		peerBlocks:        make(map[string]map[types.Hash]*block.Block),
		peerReceipts:      make(map[string]map[types.Hash]transaction.Receipts),
		peerChainTds:      make(map[string]map[types.Hash]*big.Int),
		peerMissingStates: make(map[string]map[types.Hash]bool),
	}
	tester.stateDb, _ = database.OpenMemDB()
...
// reassembly.
func (dl *downloadTester) makeChain(n int, seed byte, parent *block.Block, parentReceipts transaction.Receipts, heavy bool) ([]types.Hash, map[types.Hash]*block.Header, map[types.Hash]*block.Block, map[types.Hash]transaction.Receipts) {
	// Generate the block chain
	blocks, receipts := chainmaker.GenerateChain(params.TestChainConfig, parent, &consensus.Engine_empty{}, dl.peerDb, n, func(i int, block *chainmaker.BlockGen) {
		block.SetCoinbase(types.Address{seed})

		// If a heavy chain is requested, delay blocks to raise difficulty
		if heavy {
			block.OffsetTime(-1)
		}
		// If the block number is multiple of 3, send a bonus transaction to the blockproducer
		if parent == dl.genesis && i%3 == 0 {
			signer := transaction.MakeSigner(params.TestChainConfig, block.Number())
			actions := transaction.Actions{&transaction.Action{Contract: types.Address{}, Params: []byte{}}}
			tx, err := transaction.SignTx(transaction.NewTransaction(block.TxNonce(testAddress), actions), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
	...
	headerm := make(map[types.Hash]*block.Header, n+1)
	headerm[parent.Hash()] = parent.Header()

	blockm := make(map[types.Hash]*block.Block, n+1)
	blockm[parent.Hash()] = parent

	receiptm := make(map[types.Hash]transaction.Receipts, n+1)
...
func (dl *downloadTester) makeChainFork(n, f int, parent *block.Block, parentReceipts transaction.Receipts, balanced bool) ([]types.Hash, []types.Hash, map[types.Hash]*block.Header, map[types.Hash]*block.Header, map[types.Hash]*block.Block, map[types.Hash]*block.Block, map[types.Hash]transaction.Receipts, map[types.Hash]transaction.Receipts) {
...
func (dl *downloadTester) HasHeader(hash types.Hash, number uint64) bool {
	return dl.GetHeaderByHash(hash) != nil
}

// HasBlockAndState checks if a block and associated state is present in the testers canonical chain.
func (dl *downloadTester) HasBlockAndState(hash types.Hash) bool {
...
func (dl *downloadTester) GetHeaderByHash(hash types.Hash) *block.Header {
	dl.lock.RLock()
...
func (dl *downloadTester) GetBlockByHash(hash types.Hash) *block.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentHeader() *block.Header {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentBlock() *block.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) CurrentFastBlock() *block.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) FastSyncCommitHead(hash types.Hash) error {
...
func (dl *downloadTester) GetTdByHash(hash types.Hash) *big.Int {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) InsertHeaderChain(headers []*block.Header, checkFreq int) (int, error) {
	dl.lock.Lock()
	...
	for i, header := range headers {
		...
		dl.ownChainTd[header.Hash()] = &header.Number.IntVal
	}
	return len(headers), nil
}

// InsertChain injects a new batch of blocks into the simulated chain.
func (dl *downloadTester) InsertChain(blocks block.Blocks) (int, error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
	...
	for i, block := range blocks {
		...
		dl.ownChainTd[block.Hash()] = block.Number()
	}
	return len(blocks), nil
}

// InsertReceiptChain injects a new batch of receipts into the simulated chain.
func (dl *downloadTester) InsertReceiptChain(blocks block.Blocks, receipts []transaction.Receipts) (int, error) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) WriteConsensusData(key types.Hash, value []byte) error {
	return nil
}

// Rollback removes some recently added elements from the chain.
func (dl *downloadTester) Rollback(hashes []types.Hash) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) newPeer(id string, version int, hashes []types.Hash, headers map[types.Hash]*block.Header, blocks map[types.Hash]*block.Block, receipts map[types.Hash]transaction.Receipts) error {
...
func (dl *downloadTester) newSlowPeer(id string, version int, hashes []types.Hash, headers map[types.Hash]*block.Header, blocks map[types.Hash]*block.Block, receipts map[types.Hash]transaction.Receipts, delay time.Duration) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
	...
	if err == nil {
		...
		dl.peerHashes[id] = make([]types.Hash, len(hashes))
		copy(dl.peerHashes[id], hashes)

		dl.peerHeaders[id] = make(map[types.Hash]*block.Header)
		dl.peerBlocks[id] = make(map[types.Hash]*block.Block)
		dl.peerReceipts[id] = make(map[types.Hash]transaction.Receipts)
		dl.peerChainTds[id] = make(map[types.Hash]*big.Int)
		dl.peerMissingStates[id] = make(map[types.Hash]bool)

		genesis := hashes[len(hashes)-1]
		if header := headers[genesis]; header != nil {
			dl.peerHeaders[id][genesis] = header
			dl.peerChainTds[id][genesis] = &header.Number.IntVal
		}
		if block := blocks[genesis]; block != nil {
			dl.peerBlocks[id][genesis] = block
			dl.peerChainTds[id][genesis] = block.Number()
		}
		...
		for i := len(hashes) - 2; i >= 0; i-- {
			...
			if header, ok := headers[hash]; ok {
				...
				if _, ok := dl.peerHeaders[id][header.ParentHash]; ok {
					dl.peerChainTds[id][hash] = &header.Number.IntVal
				}
			}
			if block, ok := blocks[hash]; ok {
				dl.peerBlocks[id][hash] = block
				if _, ok := dl.peerBlocks[id][block.ParentHash()]; ok {
					dl.peerChainTds[id][hash] = block.Number()
				}
			}
...
func (dlp *downloadTesterPeer) Head() (types.Hash, *big.Int) {
	dlp.dl.lock.RLock()
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin types.Hash, amount int, skip int, reverse bool) error {
...
func (dlp *downloadTesterPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool) error {
	...
	result := make([]*block.Header, 0, amount)
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []types.Hash) error {
	dlp.waitDelay()
	...
	transactions := make([][]*transaction.Transaction, 0, len(hashes))
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []types.Hash) error {
	dlp.waitDelay()
	...
	results := make([][]*transaction.Receipt, 0, len(hashes))
	for _, hash := range hashes {
		if receipt, ok := receipts[hash]; ok {
...
func (dlp *downloadTesterPeer) RequestNodeData(hashes []types.Hash) error {
	dlp.waitDelay()
...
func (ftp *downloadTesterPeer) RequestCertificates(hashes []types.Hash) error {
	return nil
}
...
func assertOwnForkedChain(t *testing.T, tester *downloadTester, common int, lengths []int) {
	...
	if tester.downloader.mode == FastSync {
		...
		if index > 0 {
			if statedb, err := state.New(tester.ownHeaders[tester.ownHashes[index]].StateRootHash, state.NewDatabase(tester.stateDb)); statedb == nil || err != nil {
		...
		for start := time.Now(); time.Since(start) < 3*time.Second; {
			...
			if mode == FastSync {
				if receipts := len(tester.downloader.queue.receiptDonePool); receipts < cached {
					if tester.downloader.queue.resultCache[receipts].Header.Number.IntVal.Uint64() < tester.downloader.queue.fastSyncPivot {
						cached = receipts
					}
				}
			}
...
func TestInactiveDownloader62(t *testing.T) {
	...
	if err := tester.downloader.DeliverHeaders("bad peer", []*block.Header{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
	if err := tester.downloader.DeliverBodies("bad peer", [][]*transaction.Transaction{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
}
...
func TestInactiveDownloader63(t *testing.T) {
	...
	if err := tester.downloader.DeliverReceipts("bad peer", [][]*transaction.Receipt{}); err != errNoSyncActive {
		t.Errorf("error mismatch: have %v, want %v", err, errNoSyncActive)
	}
}
...
func testEmptyShortCircuit(t *testing.T, protocol int, mode SyncMode) {
	...
	tester.downloader.bodyFetchHook = func(headers []*block.Header) {
		atomic.AddInt32(&bodiesHave, int32(len(headers)))
	}
	tester.downloader.receiptFetchHook = func(headers []*block.Header) {
	...
	for hash, receipt := range receipts {
		if mode == FastSync && len(receipt) > 0 && headers[hash].Number.IntVal.Uint64() <= tester.downloader.queue.fastSyncPivot {
			receiptsNeeded++
		}
	}
...
func testInvalidHeaderRollback(t *testing.T, protocol int, mode SyncMode) {
	...
	if head := tester.CurrentHeader().Number.IntVal.Int64(); int(head) > MaxHeaderFetch {
	...
	if head := tester.CurrentHeader().Number.IntVal.Int64(); int(head) > 2*fsHeaderSafetyNet+MaxHeaderFetch {
...
func testHighTDStarvationAttack(t *testing.T, protocol int, mode SyncMode) {
	...
	tester.newPeer("attack", protocol, []types.Hash{hashes[0]}, headers, blocks, receipts)
...
func testBlockHeaderAttackerDropping(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		if err := tester.newPeer(id, protocol, []types.Hash{tester.genesis.Hash()}, nil, nil, nil); err != nil {
		...
		tester.downloader.synchroniseMock = func(string, types.Hash) error { return tt.result }
...
func (ftp *floodingTestPeer) Head() (types.Hash, *big.Int) { return ftp.peer.Head() }
func (ftp *floodingTestPeer) RequestHeadersByHash(hash types.Hash, count int, skip int, reverse bool) error {
	return ftp.peer.RequestHeadersByHash(hash, count, skip, reverse)
}
func (ftp *floodingTestPeer) RequestBodies(hashes []types.Hash) error {
	return ftp.peer.RequestBodies(hashes)
}
func (ftp *floodingTestPeer) RequestReceipts(hashes []types.Hash) error {
	return ftp.peer.RequestReceipts(hashes)
}
func (ftp *floodingTestPeer) RequestNodeData(hashes []types.Hash) error {
	return ftp.peer.RequestNodeData(hashes)
}
func (ftp *floodingTestPeer) RequestCertificates(hashes []types.Hash) error {
	return ftp.peer.RequestCertificates(hashes)
}
func (ftp *floodingTestPeer) RequestHeadersByNumber(from uint64, count, skip int, reverse bool) error {
	...
	for i := 0; i < cap(deliveriesDone); i++ {
		...
		go func() {
			ftp.tester.downloader.DeliverHeaders(peer, []*block.Header{{}, {}, {}, {}})
...
func testFastCriticalRestarts(t *testing.T, protocol int, progress bool) {
	...
	for i := 0; i < fsPivotInterval; i++ {
		tester.peerMissingStates["peer"][headers[hashes[fsMinFullBlocks+i]].StateHash] = true
	}
	...
	for i := 0; i < int(fsCriticalTrials)-1; i++ {
		...
		if i == 0 {
			...
			tester.peerMissingStates["peer"] = map[types.Hash]bool{tester.downloader.fsPivotLock.StateHash: true}
	...
	if progress {
		tester.lock.Lock()
		tester.peerMissingStates["peer"] = map[types.Hash]bool{}
		tester.lock.Unlock()
