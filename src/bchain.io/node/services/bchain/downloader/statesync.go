...
type stateReq struct {
	items    []types.Hash              // Hashes of the state items to download
	tasks    map[types.Hash]*stateTask // Download tasks to track previous attempts
...
func (d *Downloader) syncState(root types.Hash) *stateSync {
...
type stateSync struct {
	...
	tasks  map[types.Hash]*stateTask // Set of tasks currently queued for retrieval
...
func newStateSync(d *Downloader, root types.Hash) *stateSync {
	return &stateSync{
		d:       d,
		sched:   state.NewStateSync(root, d.stateDB),
		keccak:  sha3.NewKeccak256(),
		tasks:   make(map[types.Hash]*stateTask),
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < blockchain.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) assignTasks() {
	...
	for _, p := range peers {
		...
		if len(req.items) > 0 {
			logger.Trace("Requesting new batch of data", "type", "state", "count", len(req.items))
			select {
...
func (s *stateSync) fillTasks(n int, req *stateReq) {
	...
	req.items = make([]types.Hash, 0, n)
	req.tasks = make(map[types.Hash]*stateTask, n)
...
func (s *stateSync) process(req *stateReq) (bool, error) {
	...
	if progress && atomic.LoadUint32(&s.d.fsPivotFails) > 1 {
		logger.Trace("Fast-sync progressed, resetting fail counter", "previous", atomic.LoadUint32(&s.d.fsPivotFails))
...
// error occurred.
func (s *stateSync) processNodeData(blob []byte) (bool, types.Hash, error) {
