...
type TrieSyncLeafCallback func(leaf []byte, parent types.Hash) error
...
type TrieSync struct {
	...
	requests map[types.Hash]*request  // Pending requests pertaining to a key hash
	queue    *prque.Prque             // Priority queue with the pending requests
}

// NewTrieSync creates a new trie data download scheduler.
func NewTrieSync(root types.Hash, database DatabaseReader, callback TrieSyncLeafCallback) *TrieSync {
	ts := &TrieSync{
		database: database,
...
func (s *TrieSync) AddSubTrie(root types.Hash, depth int, parent types.Hash, callback TrieSyncLeafCallback) {
	...
	if local, err := decodeNode(key, blob, 0); local.Node != nil && err == nil {
		return
	}
...
// contract code).
func (s *TrieSync) AddRawEntry(hash types.Hash, depth int, parent types.Hash) {
...
func (s *TrieSync) children(req *request, object NodeIntf) ([]*request, error) {
	...
	for _, child := range children {
		...
		if req.callback != nil {
			if node, ok := (child.node.Node).(ValueNode); ok {
		...
		if node, ok := (child.node.Node).(HashNode); ok {
