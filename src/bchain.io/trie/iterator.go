...
type nodeIteratorState struct {
	...
	node    NodeIntf    // Trie node being iterated
...
func (it *nodeIterator) LeafBlob() []byte {
	if len(it.stack) > 0 {
		if node, ok := it.stack[len(it.stack)-1].node.Node.(ValueNode); ok {
			return []byte(node)
		}
	}
	panic("not at leaf")
...
func (it *nodeIterator) LeafKey() []byte {
	if len(it.stack) > 0 {
		if _, ok := it.stack[len(it.stack)-1].node.Node.(ValueNode); ok {
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.Node.(HashNode); ok {
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor types.Hash) (*nodeIteratorState, []byte, bool) {
	switch node := parent.node.Node.(type) {
	case *FullNode:
