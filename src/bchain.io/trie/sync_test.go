...
func makeTestTrie() (database.IDatabase, *Trie, map[string][]byte) {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(types.Hash{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db, _ := database.OpenMemDB()
		if req := NewTrieSync(types.BytesToHash(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	sched := NewTrieSync(types.BytesToHash(srcTrie.Root()), dstDb, nil)
