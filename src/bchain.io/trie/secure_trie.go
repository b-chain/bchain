...
func NewSecure(root types.Hash, db Database, cachelimit uint16) (*SecureTrie, error) {
	if db == nil {
...
// from the database.
func (t *SecureTrie) Commit() (root types.Hash, err error) {
	return t.CommitTo(t.trie.db)
}

func (t *SecureTrie) Hash() types.Hash {
	return t.trie.Hash()
}
...
func (t *SecureTrie) CommitTo(db DatabaseWriter) (root types.Hash, err error) {
