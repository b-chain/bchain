...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	for _, key := range db.(*database.MemDatabase).Keys() {
...
func TestIteratorContinueAfterSeekError(t *testing.T) {
	...
	db, _ := database.OpenMemDB()
	ctr, _ := New(types.Hash{}, db)
	for _, val := range testdata1 {
		ctr.Update([]byte(val.k), []byte(val.v))
	}
	root, _ := ctr.Commit()
	barNodeHash := types.HexToHash("8ff858ed9696278da87730ca641d90cbd355585d4e499e31a55b7b6cc1b7b190")
