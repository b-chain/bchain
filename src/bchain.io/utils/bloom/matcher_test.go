...
func TestMatcherWildcards(t *testing.T) {
	matcher := NewMatcher(testSectionSize, [][][]byte{
		{types.Address{}.Bytes(), types.Address{0x01}.Bytes()}, // Default address is not a wildcard
		{types.Hash{}.Bytes(), types.Hash{0x01}.Bytes()},       // Default hash is not a wildcard
		{types.Hash{0x01}.Bytes()},                              // Plain rule, sanity check
		{types.Hash{0x01}.Bytes(), nil},                         // Wildcard suffix, drop rule
		{nil, types.Hash{0x01}.Bytes()},                         // Wildcard prefix, drop rule
