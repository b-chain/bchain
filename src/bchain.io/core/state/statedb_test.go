...
func TestUpdateLeaks(t *testing.T) {
	...
	state, _ := New(types.Hash{}, NewDatabase(db))
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := database.OpenMemDB()
	finalDb, _ := database.OpenMemDB()
	transState, _ := New(types.Hash{}, NewDatabase(transDb))
	...
	if _, _,  err := transState.CommitTo(transDb, false); err != nil {
		t.Fatalf("failed to commit transition state: %v", err)
	}
	if _,_, err := finalState.CommitTo(finalDb, false); err != nil {
...
func TestCopy(t *testing.T) {
	...
	mem, _ := database.OpenMemDB()
	orig, _ := New(types.Hash{}, NewDatabase(mem))
	...
	actions := []testAction{
		...
		},
		{
			...
			fn: func(a testAction, s *StateDB) {
				...
				for i := 0; i < 2; i++ {
					data[i] = make([]byte, 2)
					binary.BigEndian.PutUint16(data[i], uint16(a.args[0]))
				}
				s.AddLog(&transaction.Log{Address: addr, Data: data})
			},
			args: make([]int64, 1),
		},
	}
...
func (test *snapshotTest) run() bool {
	...
	for sindex--; sindex >= 0; sindex-- {
		checkstate, _ := New(types.Hash{}, NewDatabase(db))
...
func (s *StateSuite) TestTouchDelete(c *check.C) {
	...
	root, _, _ := s.state.CommitTo(s.db, false)
	s.state.Reset(root)

	snapshot := s.state.Snapshot()
	s.state.SetNonce(types.Address{}, 1000)
...
func TestContract(t *testing.T) {
	...
	var count = byte(10)
	expIID := make(map[types.Address]types.Hash, count)
	expCreator := make(map[types.Address]types.Address, count)
	// Update it with some accounts
	for i := byte(0); i < count; i++ {
		addr := types.BytesToAddress([]byte{i})

		expIID[addr] = types.Hash{byte(i)}
		state.SetInterpreterID(addr, expIID[addr])

		expCreator[addr] = types.Address{byte(i)}
		state.SetCreator(addr, expCreator[addr])
	...
	// Commit and cross check the databases.
	if _,_, err := state.CommitTo(db, true); err != nil {
		t.Fatalf("failed to commit transition state: %v", err)
	}
	...
	for _, key := range db.Keys() {
		if value, err := db.Get(key); err != nil {
			t.Fatalf("State leaked into database: %s -> %s", types.Bytes2Hex(key), types.BytesToAddress(value))
		}
	}

	for i := byte(0); i < count; i++ {
		st := state.getStateObject(types.BytesToAddress([]byte{i}))
		if expIID[st.Address()] != st.InterpreterID() {
			t.Fatal("expected InterpreterID is:", expIID[st.Address()], "not:", st.InterpreterID())
		}
		if expCreator[st.Address()] != st.Creator() {
			t.Fatal("expected Creator is:", expCreator[st.Address()], "not:", st.Creator())
		}
	}
