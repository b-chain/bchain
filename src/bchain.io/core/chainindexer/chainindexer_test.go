...
func testChainIndexer(t *testing.T, count int) {
	...
	for i := 0; i < count; i++ {
		...
		backends[i].indexer = NewChainIndexer(db, database.NewTable(db, string([]byte{byte(i)})), backends[i], sectionSize, confirmsReq, 0, fmt.Sprintf("indexer-%d", i))
	...
	inject := func(number uint64) {
		header := &block.Header{Number: types.NewBigInt(*big.NewInt(int64(number))), Cdata: block.ConsensusData{Para: big.NewInt(rand.Int63()).Bytes()}}
		if number > 0 {
			header.ParentHash = blockchain.GetCanonicalHash(db, number-1)
		}
		blockchain.WriteHeader(db, header)
		blockchain.WriteCanonicalHash(db, header.Hash(), number)
	}
...
func (b *testChainIndexBackend) Reset(section uint64, prevHead types.Hash) error {
	b.section = section
	b.headerCnt = 0
	return nil
}

func (b *testChainIndexBackend) Process(header *block.Header) {
	b.headerCnt++
	...
	case b.processCh <- header.Number.IntVal.Uint64():
	}
}
