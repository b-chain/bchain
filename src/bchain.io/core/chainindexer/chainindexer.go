...
type ChainIndexerBackend interface {
	...
	Reset(section uint64, prevHead types.Hash) error

	// Process crunches through the next header in the chain segment. The caller
	// will ensure a sequential order of headers.
	Process(header *block.Header)
...
type ChainIndexer struct {
	chainDb  database.IDatabase      // Chain database to index the data from
	indexDb  database.IDatabase      // Prefixed table-view of the db to write index metadata into
...
// queue.
func (c *ChainIndexer) eventLoop(currentHeader *block.Header, events chan core.ChainEvent, sub event.Subscription) {
	...
	c.newHead(currentHeader.Number.IntVal.Uint64(), false)
	...
	for {
		...
		case ev, ok := <-events:
			...
			if header.ParentHash != prevHash {
				...
				if h := blockchain.FindCommonAncestor(c.chainDb, prevHeader, header); h != nil {
					c.newHead(h.Number.IntVal.Uint64(), true)
				}
			}
			c.newHead(header.Number.IntVal.Uint64(), false)
			...
			if c.knownSections > c.storedSections {
				...
				if time.Since(updated) > 8*time.Second {
					if c.knownSections > c.storedSections+1 {
						updating = true
						logger.Info("Upgrading chain index", "percentage", c.storedSections*100/c.knownSections)
					}
					updated = time.Now()
				}
				...
				if err != nil {
					logger.Error("Section processing failed", "error", err)
				}
				c.lock.Lock()
				...
				if err == nil && oldHead == c.SectionHead(section-1) {
					...
					if c.storedSections == c.knownSections && updating {
						updating = false
						logger.Info("Finished upgrading chain index")
					}

					c.cascadedHead = c.storedSections*c.sectionSize - 1
					for _, child := range c.children {
						logger.Trace("Cascading chain index update", "head", c.cascadedHead)
						child.newHead(c.cascadedHead, false)
					}
				} else {
					// If processing failed, don't retry until further notification
					logger.Debug("Chain index processing failed", "section", section, "err", err)
...
func (c *ChainIndexer) processSection(section uint64, lastHead types.Hash) (types.Hash, error) {
	logger.Trace("Processing new chain section", "section", section)
	...
	for number := section * c.sectionSize; number < (section+1)*c.sectionSize; number++ {
		hash := blockchain.GetCanonicalHash(c.chainDb, number)
		if hash == (types.Hash{}) {
			return types.Hash{}, fmt.Errorf("canonical block #%d unknown", number)
		}
		header := blockchain.GetHeader(c.chainDb, hash, number)
		if header == nil {
	...
	if err := c.backend.Commit(); err != nil {
		logger.Error("Section commit failed", "error", err)
		return types.Hash{}, err
	}
	return lastHead, nil
}
