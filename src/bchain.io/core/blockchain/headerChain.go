...
type HeaderChain struct {
	...
	currentHeader     *block.Header // Current head of the header chain (may be above the block chain!)
	currentHeaderHash types.Hash    // Hash of the current head of the header chain (prevent recomputing all the time)
...
func NewHeaderChain(chainDb database.IDatabase, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if head := GetHeadBlockHash(chainDb); head != (types.Hash{}) {
...
func (hc *HeaderChain) WriteHeader(header *block.Header) (status WriteStatus, err error) {
	...
	currentCanonicaNumber := hc.currentHeader.Number.IntVal.Uint64()

	if err := WriteHeader(hc.chainDb, header); err != nil {
		logger.Critical("Failed to write header content", "err", err)
	}
	// if header number is bigger,
	if number > currentCanonicaNumber {
		// Overwrite any stale canonical number assignments
		var (
			headHash   = header.ParentHash
			headNumber = header.Number.IntVal.Uint64() - 1
		...
		for GetCanonicalHash(hc.chainDb, headNumber) != headHash {
			...
			headNumber = headHeader.Number.IntVal.Uint64() - 1
			headHeader = hc.GetHeader(headHash, headNumber)
		}
		// Extend the canonical chain with the new header
		if err := WriteCanonicalHash(hc.chainDb, hash, number); err != nil {
			logger.Critical("Failed to insert header number", "err", err)
		}
		if err := WriteHeadHeaderHash(hc.chainDb, hash); err != nil {
			logger.Critical("Failed to insert head header hash", "err", err)
		}
		hc.currentHeaderHash, hc.currentHeader = hash, block.CopyHeader(header)
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*block.Header, checkFreq int) (int, error) {
	...
	for i := 1; i < len(chain); i++ {
		if chain[i].Number.IntVal.Uint64() != chain[i-1].Number.IntVal.Uint64()+1 || chain[i].ParentHash != chain[i-1].Hash() {
			// Chain broke ancestry, log a messge (programming error) and skip insertion
			logger.Error("Non contiguous header insert", "number", chain[i].Number.IntVal.String(), "hash", chain[i].Hash().String(),
				"parent", chain[i].ParentHash.String(), "prevnumber", chain[i-1].Number.IntVal.String(), "prevhash", chain[i-1].Hash().String())

			return 0, fmt.Errorf("non contiguous insert: item %d is #%s [%x…], item %d is #%s [%x…] (parent [%x…])", i-1, chain[i-1].Number,
...
func (hc *HeaderChain) InsertHeaderChain(chain []*block.Header, writeHeader WhCallback, start time.Time) (int, error) {
	...
	for i, header := range chain {
		...
		if hc.HasHeader(header.Hash(), header.Number.IntVal.Uint64()) {
			stats.ignored++
			continue
		}
	...
	logger.Info("Imported new block headers", "count", stats.processed, "elapsed", common.PrettyDuration(time.Since(start)),
		"number", last.Number.IntVal.String(), "hash", last.Hash().String(), "ignored", stats.ignored)
...
func (hc *HeaderChain) GetBlockHashesFromHash(hash types.Hash, max uint64) []types.Hash {
	...
	for i := uint64(0); i < max; i++ {
		...
		if header = hc.GetHeader(next, header.Number.IntVal.Uint64()-1); header == nil {
			break
		}
		chain = append(chain, next)
		if header.Number.IntVal.Sign() == 0 {
			break
		}
	}
	return chain
...
func (hc *HeaderChain) SetHead(head uint64, delFn DeleteCallback) {
	...
	if hc.currentHeader != nil {
		height = hc.currentHeader.Number.IntVal.Uint64()
	}

	for hc.currentHeader != nil && hc.currentHeader.Number.IntVal.Uint64() > head {
		hash := hc.currentHeader.Hash()
		num := hc.currentHeader.Number.IntVal.Uint64()
		if delFn != nil {
			delFn(hash, num)
		}
		DeleteHeader(hc.chainDb, hash, num)
		hc.currentHeader = hc.GetHeader(hc.currentHeader.ParentHash, hc.currentHeader.Number.IntVal.Uint64()-1)
	}
	...
	if err := WriteHeadHeaderHash(hc.chainDb, hc.currentHeaderHash); err != nil {
		logger.Critical("Failed to reset head header hash", "err", err)
	}
}
