...
var (
	...
	ErrChainId = errors.New("ChainID should not be nil")
)
...
func (ks *KeyStore) SignHeader( h *block.Header,chainID *big.Int) (*block.Header, error) {
	...
	bchainBaseAccouts := ks.wallets[0]
	if len(bchainBaseAccouts.Accounts()) < 0 {
		return nil , errors.New("no accounts in wallet")
	}
	a := bchainBaseAccouts.Accounts()[0]
...
func (ks *KeyStore) SignTxWithPassphrase(a accounts.Account, passphrase string, tx *transaction.Transaction, chainID *big.Int) (*transaction.Transaction, error) {
	...
	// Depending on the presence of the chain ID, sign with EIP155 or homestead
	if chainID != nil {
		return transaction.SignTx(tx, transaction.NewMSigner(chainID), key.PrivateKey)
	}
	return nil,ErrChainId
}
...
// ImportPreSaleKey decrypts the given Bchain presale wallet and stores
