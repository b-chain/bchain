...
func newTestTransport(id discover.NodeID, fd net.Conn) transport {
	wrapped := newMsgpx(fd).(*msgpx)
	wrapped.rw = newMsgpxFrameRW(fd, secrets{
		MAC:        zero16,
		AES:        zero16,
		IngressMAC: sha3.NewKeccak256(),
		EgressMAC:  sha3.NewKeccak256(),
	})
	return &testTransport{id: id, msgpx: wrapped}
}
...
func TestServerDial(t *testing.T) {
	...
	srv.AddPeer(&discover.Node{ID: remid, IP: *types.NewIP(tcpAddr.IP), TCP: uint16(tcpAddr.Port)})
