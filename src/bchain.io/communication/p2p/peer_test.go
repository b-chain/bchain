...
func TestPeerProtoReadMsg(t *testing.T) {
	proto := Protocol{
		...
		Run: func(peer *Peer, rw MsgReadWriter) error {
			if err := ExpectMsg(rw, 2, []uint{1, 2, 3}); err != nil {
				t.Error(err)
			}
			if err := ExpectMsg(rw, 3, []uint{2}); err != nil {
				t.Error(err)
			}
			if err := ExpectMsg(rw, 4, []uint{3}, []uint{3, 4, 5}); err != nil {
				t.Error(err)
			}
			return nil
		},
	...
	Send(rw, baseProtocolLength+2, []uint{1, 2, 3})
	Send(rw, baseProtocolLength+3, []uint{2})
	Send(rw, baseProtocolLength+4, []uint{3}, []uint{3, 4, 5})
...
func TestPeerProtoEncodeMsg(t *testing.T) {
	...
	if err := ExpectMsg(rw, 17, "foo", "bar"); err != nil {
		t.Error(err)
	}
}
...
func TestPeerPing(t *testing.T) {
	...
	if err := ExpectMsg(rw, pongMsg); err != nil {
		t.Error(err)
	}
}
