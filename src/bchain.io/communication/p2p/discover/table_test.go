...
var lookupTestnet = &preblockProduceTestnet{
...
type preblockProduceTestnet struct {
	target    NodeID
	targetSha types.Hash // sha3(target)
	dists     [hashBits + 1][]NodeID
}

func (tn *preblockProduceTestnet) findnode(toid NodeID, toaddr *net.UDPAddr, target NodeID) ([]*Node, error) {
...
func (*preblockProduceTestnet) close()                                      {}
func (*preblockProduceTestnet) waitping(from NodeID) error                  { return nil }
func (*preblockProduceTestnet) ping(toid NodeID, toaddr *net.UDPAddr) error { return nil }

// block produce generates a testnet struct literal with nodes at
// various distances to the given target.
func (n *preblockProduceTestnet) blockProduce(target NodeID) {
	n.target = target
	...
	fmt.Println("&preblockProduceTestnet{")
	fmt.Printf("	target: %#v,\n", n.target)
