...
//
// The Node Discovery protocol provides a way to find Msgpx nodes that
...
func (tab *Table) doRefresh(done chan struct{}) {
	...
	if len(seeds) == 0 {
		logger.Debug("No discv4 seed nodes found")
	}
	/*for _, n := range seeds {
		age := logger.Lazy{Fn: func() time.Duration { return time.Since(tab.db.lastPong(n.ID)) }}
		logger.Trace("Found seed node in database", "id", n.ID, "addr", n.addr(), "age", age)
	}*/
	tab.mutex.Lock()
	tab.stuff(seeds)
	tab.mutex.Unlock()
