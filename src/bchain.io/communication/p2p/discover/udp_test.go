...
var (
	...
	testRemote         = RpcEndpoint{IP: *types.NewIP(net.ParseIP("1.1.1.1").To4()), UDP: 1, TCP: 2}
	testLocalAnnounced = RpcEndpoint{IP: *types.NewIP(net.ParseIP("2.2.2.2").To4()), UDP: 3, TCP: 4}
	testLocal          = RpcEndpoint{IP: *types.NewIP(net.ParseIP("3.3.3.3").To4()), UDP: 5, TCP: 6}
)
...
func TestUDP_findnodeMultiReply(t *testing.T) {
	...
	list := []*Node{
		MustParseNode("bnode://ba85011c70bcc5c04d8607d3a0ed29aa6179c092cbdda10d5d32684fb33ed01bd94f588ca8f91ac48318087dcb02eaf36773a7a453f0eedd6742af668097b29c@10.0.1.16:30303?discport=30304"),
		MustParseNode("bnode://81fa361d25f157cd421c60dcc28d8dac5ef6a89476633339c5df30287474520caca09627da18543d9079b5b288698b542d56167aa5c09111e55acdbbdf2ef799@10.0.1.16:30303"),
		MustParseNode("bnode://9bffefd833d53fac8e652415f4973bee289e8b1a5c6c4cbe70abf817ce8a64cee11b823b66a987f51aaa9fba0d6a91b3e6bf0d5a5d1042de8e9eeea057b217f8@10.0.1.36:30301?discport=17"),
		MustParseNode("bnode://1b5b4aa662d7cb44a7221bfba67302590b643028197a7d5214790f3bac7aaa4a3241be9e83c09cf1f6c69d007c634faae3dc1b1221793e8446c0b3a09de65960@10.0.1.16:30303"),
	}
...
func TestUDP_successfulPing(t *testing.T) {
	...
	test.waitPacketOut(func(p *Pong) {
		...
		wantTo := RpcEndpoint{
			...
			IP: *types.NewIP(test.remoteaddr.IP), UDP: uint16(test.remoteaddr.Port),
	...
	case n := <-added:
		...
		if !n.IP.Get().Equal(test.remoteaddr.IP) {
...
var testPackets = []struct {
...
}{
	{
		...
		wantPacket: &Ping{
			Version:    4,
			From:       RpcEndpoint{*types.NewIP(net.ParseIP("127.0.0.1").To4()), 3322, 5544},
			To:         RpcEndpoint{*types.NewIP(net.ParseIP("::1")), 2222, 3333},
	...
	},
	{
		...
		wantPacket: &Ping{
			Version:    555,
			From:       RpcEndpoint{*types.NewIP(net.ParseIP("2001:db8:3c4d:15::abcd:ef12")), 3322, 5544},
			To:         RpcEndpoint{*types.NewIP(net.ParseIP("2001:db8:85a3:8d3:1319:8a2e:370:7348")), 2222, 33338},
			Expiration: 1136239445,
	...
	},
	{
		...
		wantPacket: &Pong{
			To:         RpcEndpoint{*types.NewIP(net.ParseIP("2001:db8:85a3:8d3:1319:8a2e:370:7348")), 2222, 33338},
			ReplyTok:   types.Hex2Bytes("fbc914b16819237dcd8801d7e53f69e9719adecb3cc0e790c57e91ca4461c954"),
			Expiration: 1136239445,
			...
			Nodes: []RpcNode{
				{
					...
					IP:  *types.NewIP(net.ParseIP("99.33.22.55").To4()),
					UDP: 4444,
					TCP: 4445,
				},
				{
					...
					IP:  *types.NewIP(net.ParseIP("1.2.3.4").To4()),
					UDP: 1,
					TCP: 1,
				},
				{
					...
					IP:  *types.NewIP(net.ParseIP("2001:db8:3c4d:15::abcd:ef12")),
					UDP: 3333,
					TCP: 3333,
				},
				{
					...
					IP:  *types.NewIP(net.ParseIP("2001:db8:85a3:8d3:1319:8a2e:370:7348")),
					UDP: 999,
					TCP: 1000,
				},
			},
...
// TODO: the test data(testPackets) must update !!! so don't worry
func TestForwardCompatibility(t *testing.T) {
	t.Skipf("The test data(testPackets) must update !!!")
...
func (c *dgramPipe) LocalAddr() net.Addr {
	return &net.UDPAddr{IP: testLocal.IP.Get(), Port: int(testLocal.UDP)}
}
