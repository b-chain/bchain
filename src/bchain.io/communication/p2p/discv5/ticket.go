...
func pongToTicket(localTime mclock.AbsTime, topics []Topic, node *Node, p *ingressPacket) (*ticket, error) {
	...
	if msgpHashTopics(topics) != p.data.(*Pong).TopicHash {
...
func ticketToPong(t *ticket, pong *Pong) {
	...
	pong.TopicHash = msgpHashTopics(t.topics)
...
func (s *ticketStore) addTopic(topic Topic, register bool) {
	logger.Trace("Adding discovery topic.", "topic", topic, ", register", register)
...
func (s *ticketStore) removeRegisterTopic(topic Topic) {
	logger.Trace("Removing discovery topic.", "topic", topic)
	if s.tickets[topic] == nil {
		logger.Warn("Removing non-existent discovery topic.", "topic", topic)
		return
	}
...
func (s *ticketStore) nextRegisterLookup() (lookupInfo, time.Duration) {
	...
	for len(s.regQueue) > 0 {
		...
		if s.tickets[topic].nextLookup < mclock.Now() {
			...
			logger.Trace("Found discovery topic to register.", "topic", topic, ", target", next.target, ", delay", delay)
			return next, delay
		}
	}
	// No registration topics found or all exhausted, sleep
	delay := 40 * time.Second
	logger.Trace("No topic found to register.", "delay", delay)
...
func (s *ticketStore) ticketsInWindow(topic Topic) []ticketRef {
	...
	if s.tickets[topic] == nil {
		logger.Warn("Listing non-existing discovery tickets.", "topic", topic)
		return nil
	}
	...
	logger.Trace("Retrieved discovery registration tickets.", "topic", topic, ", from", s.lastBucketFetched, ", tickets", len(tickets))
	return tickets
}
...
func (s *ticketStore) addTicketRef(r ticketRef) {
	...
	if tickets == nil {
		logger.Warn("Adding ticket to non-existent topic.", "topic", topic)
		return
	}
...
func (s *ticketStore) nextFilteredTicket() (*ticketRef, time.Duration) {
	...
	for {
		...
		logger.Trace("Found discovery ticket to register.", "node", ticket.t.node, ", serial", ticket.t.serial, ", wait", wait)
...
func (s *ticketStore) removeTicketRef(ref ticketRef) {
	logger.Trace("Removing discovery ticket reference.", "node", ref.t.node.ID, ", serial", ref.t.serial)

	topic := ref.topic()
	tickets := s.tickets[topic]

	if tickets == nil {
		logger.Warn("Removing tickets from unknown topic.", "topic", topic)
		return
	}
...
func (s *ticketStore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
	logger.Trace("Adding discovery ticket.", "node", ticket.node.ID, ", serial", ticket.serial)
...
func (s *ticketStore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
		logger.Trace("Retrieving node ticket.", "node", node.ID, ", serial", nil)
	} else {
		logger.Trace("Retrieving node ticket.", "node", node.ID, ", serial", s.nodes[node].serial)
	}
	return s.nodes[node]
}
...
func (s *ticketStore) gotTopicNodes(from *Node, hash types.Hash, nodes []RpcNode) (timeout bool) {
	...
	for _, node := range nodes {
		ip := node.IP
		if ip.Get().IsUnspecified() || ip.Get().IsLoopback() {
			ip = from.IP
		}
		n := NewNode(node.ID, ip.Get(), node.UDP-1, node.TCP-1) // subtract one from port while discv5 is running in test mode on UDPport+1
		select {
		case chn <- n:
		default:
			return false
