...
func (db *nodeDB) storeMsgp(key []byte, val interface{}) error {
	encodable, ok := val.(msgp.Encodable)
	if !ok {
		logger.Error("Can't convert to msgp.Encodable")
		return errors.New("Can't convert to msgp.Encodable")
	}
	...
	return db.lvl.Put(key, buf.Bytes(), nil)
}

func (db *nodeDB) fetchMsgp(key []byte, val interface{}) error {
	blob, err := db.lvl.Get(key, nil)
	if err != nil {
		return err
	}

	decodable, ok := val.(msgp.Decodable)
	if !ok {
		logger.Error("Can't convert to msgp.Decodable")
		return fmt.Errorf("Can't convert to msgp.Decodable")
	}

	err = msgp.Decode(bytes.NewBuffer(blob), decodable)
	if err == nil {
		logger.Warn(fmt.Sprintf("key %x (%T) %v", key, val, err))
	}
...
func (db *nodeDB) node(id NodeID) *Node {
	var node Node
	if err := db.fetchMsgp(makeKey(id, nodeDBDiscoverRoot), &node); err != nil {
		return nil
	}
...
func (db *nodeDB) updateNode(node *Node) error {
	return db.storeMsgp(makeKey(node.ID, nodeDBDiscoverRoot), node)
}
	...
	for {
		...
		case <-tick.C:
			if err := db.expireNodes(); err != nil {
				logger.Error(fmt.Sprintf("Failed to expire nodedb items: %v", err))
			}
		case <-db.quit:
			return
		}
...
func (db *nodeDB) localEndpoint(id NodeID) *RpcEndpoint {
	var ep RpcEndpoint
	if err := db.fetchMsgp(makeKey(id, nodeDBDiscoverLocalEndpoint), &ep); err != nil {
		return nil
	}
	return &ep
}
...
func (db *nodeDB) updateLocalEndpoint(id NodeID, ep RpcEndpoint) error {
	return db.storeMsgp(makeKey(id, nodeDBDiscoverLocalEndpoint), &ep)
}
...
func nextNode(it iterator.Iterator) *Node {
	for end := false; !end; end = !it.Next() {
		...
		var n Node
		if err := msgp.Decode(bytes.NewBuffer(it.Value()), &n); err != nil {
			logger.Warn(fmt.Sprintf("invalid node %x: %v", id, err))
			continue
		}
		return &n
	}
