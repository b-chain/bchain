...
// The nodes were obtained by running testnet block produce with a random NodeID as target.
var lookupTestnet = &preproducedTestnet{
	...
	targetSha: types.Hash{0x5c, 0x94, 0x4e, 0xe5, 0x1c, 0x5a, 0xe9, 0xf7, 0x2a, 0x95, 0xec, 0xcb, 0x8a, 0xed, 0x3, 0x74, 0xee, 0xcb, 0x51, 0x19, 0xd7, 0x20, 0xcb, 0xea, 0x68, 0x13, 0xe8, 0xe0, 0xd6, 0xad, 0x92, 0x61},
	dists: [257][]NodeID{
		240: {
...
type preproducedTestnet struct {
	target    NodeID
...
func (tn *preproducedTestnet) sendFindnode(to *Node, target NodeID) {
	panic("sendFindnode called")
}

func (tn *preproducedTestnet) sendFindnodeHash(to *Node, target types.Hash) {
...
func (tn *preproducedTestnet) sendPing(to *Node, addr *net.UDPAddr, topics []Topic) []byte {
	injectResponse(tn.net, to, pongPacket, &Pong{ReplyTok: []byte{1}})
	return []byte{1}
}

func (tn *preproducedTestnet) send(to *Node, ptype nodeEvent, data interface{}) (hash []byte) {
	switch ptype {
	case pingPacket:
		injectResponse(tn.net, to, pongPacket, &Pong{ReplyTok: []byte{1}})
	case pongPacket:
...
func (tn *preproducedTestnet) sendNeighbours(to *Node, nodes []*Node) {
	panic("sendNeighbours called")
}

func (tn *preproducedTestnet) sendTopicQuery(to *Node, topic Topic) {
	panic("sendTopicQuery called")
}

func (tn *preproducedTestnet) sendTopicNodes(to *Node, queryHash types.Hash, nodes []*Node) {
	panic("sendTopicNodes called")
}

func (tn *preproducedTestnet) sendTopicRegister(to *Node, topics []Topic, idx int, pong []byte) {
	panic("sendTopicRegister called")
}

func (*preproducedTestnet) Close() {}

func (*preproducedTestnet) localAddr() *net.UDPAddr {
	return &net.UDPAddr{IP: net.ParseIP("10.0.1.1"), Port: 40000}
}

// block procduce generates a testnet struct literal with nodes at
// various distances to the given target.
func (n *preproducedTestnet) blockproduce(target NodeID) {
	n.target = target
	...
	fmt.Println("&preproducedTestnet{")
	fmt.Printf("	target: %#v,\n", n.target)
