...
loop:
	for {
		...
		select {
		case <-net.closeReq:
			logger.Trace("<-net.closeReq")
			break loop

		// Ingress packet handling.
		case pkt := <-net.read:
			//fmt.Println("read", pkt.ev)
			logger.Trace("<-net.read")
		...
		case req := <-net.topicRegisterReq:
			...
			if topicRegisterLookupTarget.target == (types.Hash{}) {
				logger.Trace("topicRegisterLookupTarget == null")
		...
		case nodes := <-topicRegisterLookupDone:
			logger.Trace("<-topicRegisterLookupDone")
		...
		case <-topicRegisterLookupTick.C:
			logger.Trace("<-topicRegisterLookupTick")
			if (topicRegisterLookupTarget.target == types.Hash{}) {
				target, delay := net.ticketStore.nextRegisterLookup()
				topicRegisterLookupTarget = target
				topicRegisterLookupTick.Reset(delay)
		...
		case req := <-net.topicSearchReq:
			if refreshDone == nil {
				logger.Trace("<-net.topicSearchReq")
	...
	logger.Debug(fmt.Sprintf("shutting down"))
	if net.conn != nil {
		net.conn.Close()
	}
	if refreshDone != nil {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	if len(seeds) == 0 {
		logger.Trace("no seed nodes found")
		close(done)
		return
	}
	for _, n := range seeds {
...
func (net *Network) internNode(pkt *ingressPacket) *Node {
	if n := net.nodes[pkt.remoteID]; n != nil {
		n.IP = *types.NewIP(pkt.remoteAddr.IP)
...
func (net *Network) internNodeFromDB(dbn *Node) *Node {
	...
	n := NewNode(dbn.ID, dbn.IP.Get(), dbn.UDP, dbn.TCP)
	n.state = unknown
	net.nodes[n.ID] = n
	return n
}
...
func (net *Network) internNodeFromNeighbours(sender *net.UDPAddr, rn RpcNode) (n *Node, err error) {
	...
	if n == nil {
		...
		if net.netrestrict != nil && !net.netrestrict.Contains(n.IP.Get()) {
	...
	if !n.IP.Get().Equal(rn.IP.Get()) || n.UDP != rn.UDP || n.TCP != rn.TCP {
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	logger.Trace("Pinging remote node.", "node", n.ID)
...
func (net *Network) handlePing(n *Node, pkt *ingressPacket) {
	logger.Trace("Handling remote ping.", "node", n.ID)
	ping := pkt.data.(*Ping)
	n.TCP = ping.From.TCP
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	logger.Trace("Handling known pong.", "node", n.ID)
	...
	if err == nil {
		...
		logger.Trace("Failed to convert pong to ticket.", "err", err)
	}
	n.pingEcho = nil
	n.pingTopics = nil
	return err
...
func (net *Network) checkTopicRegister(data *TopicRegister) (*Pong, error) {
	...
	if msgpHashTopics(data.Topics) != pongpkt.data.(*Pong).TopicHash {
...
func msgpHashTopics(topics []Topic) (h types.Hash) {
	hw := sha3.NewKeccak256()
	for _, topic := range topics {
		msgp.Encode(hw, topic)
	}
	hw.Sum(h[:0])
	return h
}
...
func (net *Network) handleNeighboursPacket(n *Node, pkt *ingressPacket) error {
	...
	for i, rn := range req.Nodes {
		...
		if err != nil {
			logger.Debug(fmt.Sprintf("invalid neighbour (%v) from %x@%v: %v", rn.IP, n.ID[:8], pkt.remoteAddr, err))
			continue
		}
		nodes[i] = nn
