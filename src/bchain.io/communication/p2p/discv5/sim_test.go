...
func (st *simTransport) sendPing(remote *Node, remoteAddr *net.UDPAddr, topics []Topic) []byte {
	...
	st.sendPacket(remote.ID, ingressPacket{
		...
		data: &Ping{
			Version:    4,
			From:       RpcEndpoint{IP: *types.NewIP(st.senderAddr.IP), UDP: uint16(st.senderAddr.Port), TCP: 30303},
			To:         RpcEndpoint{IP: *types.NewIP(remoteAddr.IP), UDP: uint16(remoteAddr.Port), TCP: 30303},
...
func (st *simTransport) sendPong(remote *Node, pingHash []byte) {
	...
	st.sendPacket(remote.ID, ingressPacket{
		...
		ev:         pongPacket,
		data: &Pong{
			To:         RpcEndpoint{IP: *types.NewIP(raddr.IP), UDP: uint16(raddr.Port), TCP: 30303},
			ReplyTok:   pingHash,
			Expiration: uint64(time.Now().Unix() + int64(expiration)),
		},
	})
