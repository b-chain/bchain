...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout bchain_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for BchainSubscribe requests
}
...
// BchainSubscribe registers a subscripion under the "bchain" namespace.
func (c *Client) BchainSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "bchain", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. BchainSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through BchainSubscribe.
